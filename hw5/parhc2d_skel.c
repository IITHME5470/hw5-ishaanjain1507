...
void get_rhs(int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double dx, double dy, double *xleftghost, double *xrightghost, double *ybotghost, double *ytopghost, double kdiff, double *x, double *y, double **T, double **rhs)
{
	...
	if (ienglob == nxglob - 1) // processors adjacent to the right end of the domain
		...
		for (j = 1; j < ny - 1; j++)
			rhs[i][j] = kdiff * (xrightghost[j] + T[i - 1][j] - 2 * T[i][j]) / dxsq +
						kdiff * (T[i][j + 1] + T[i][j - 1] - 2 * T[i][j]) / dysq;
	...
	if (jstglob == 0)
		...
		for (i = 1; i < nx - 1; i++)
			rhs[i][j] = kdiff * (T[i + 1][j] + T[i - 1][j] - 2 * T[i][j]) / dxsq +
						kdiff * (T[i][j + 1] + ybotghost[i] - 2 * T[i][j]) / dysq;
	...
	if (jenglob == nyglob - 1)
		...
		for (i = 1; i < nx - 1; i++)
			rhs[i][j] = kdiff * (T[i + 1][j] + T[i - 1][j] - 2 * T[i][j]) / dxsq +
						kdiff * (ytopghost[i] + T[i][j - 1] - 2 * T[i][j]) / dysq;
	...
	if (istglob == 0 || jstglob == 0)
		...
		rhs[i][j] = kdiff * (T[i + 1][j] + xleftghost[j] - 2 * T[i][j]) / dxsq +
					kdiff * (T[i][j + 1] + ybotghost[i] - 2 * T[i][j]) / dysq;
...
void output_soln(int rank, int nx, int ny, int it, double tcurr, double *x, double *y, double **T)
{
	...
	sprintf(fname, "data/T_x_y_%06d_%04d.dat", it, rank);
	// printf("\n%s\n", fname);
